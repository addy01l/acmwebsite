...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.PROD ? 'https://acmwebsite.onrender.com/api/' : 'http://127.0.0.1:8000/api/')}admin/stats/`, {
  ...
  const fetchEvents = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.PROD ? 'https://acmwebsite.onrender.com/api/' : 'http://127.0.0.1:8000/api/')}events/`);
      const data = await res.json();
      setEvents(data);
    } catch (err) {
  ...
  const handleDeleteEvent = async (id: string) => {
    ...
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || (import.meta.env.PROD ? 'https://acmwebsite.onrender.com/api/' : 'http://127.0.0.1:8000/api/')}events/${id}/`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
